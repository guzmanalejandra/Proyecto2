...
def generateDatabase():
    ...
    with open('BaseDatos.csv') as csv_file:
        ...
        for row in csv_reader:
            userDict[row[0]] = [row[0], row[2],row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14], row[15]]
...
def generate_user(user_preferences: dict):
    user = Node("NUser",usuario = user_preferences["usuario"],contra = user_preferences["contrasena"] ,sexo = user_preferences["sexo"]
                , orientacion = user_preferences["orientacion"],tiene_pareja =user_preferences["tiene_pareja"],app_citas = user_preferences["app_citas"]
                , dificultad_citas = user_preferences["dificultad_citas"], importancia_hobbies = user_preferences["importancia_hobbies"]
                , importancia_estudios = user_preferences["importancia_estudios"], gusto_musical = user_preferences["gusto_musical"]
                ...
                , vida_profesional = user_preferences["vida_profesional"],gustos_similares = user_preferences["gustos_similares"])
...
def deleteUser(nombre: str, contraseña: str, matcher: NodeMatcher):
    with open('BaseNUsers.csv', 'rb') as inp, open('BaseNUsers.csv', 'wb') as out:
        ...
        out.close()
            #try:
             #   user_preferences[row[0]] = [row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10],
                                    #row[11], row[12], row[13], row[14], row[15]]
           # except IndexError:
            #    pass
        
    #db.delete(matcher.match("NUser", usuario= nombre, contra= contraseña))
    #except:
...
def generateNUsers():
    ...
    # we open the csv file and create a dictionary of key = character names and value = list with attributes
    with open('BaseNUsers.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        user_preferences = {}
    ...
    for user in user_preferences:
        nombre = user_preferences[user][0]
        contrasena= user_preferences[user][1]
        sexo1 = user_preferences[user][2]
        os = user_preferences[user][3]
        pareja = user_preferences[user][4]
        apps = user_preferences[user][5]
        dificultad = user_preferences[user][6]
        importancia = user_preferences[user][7]
        imp = user_preferences[user][8]
        gusto = user_preferences[user][9]
        region = user_preferences[user][10]
        gustoDif = user_preferences[user][11]
        Habits = user_preferences[user][12]
        goals = user_preferences[user][13]
        prof = user_preferences[user][14]
        similar = user_preferences[user][15]
        UserDict = {"usuario": nombre ,"contrasena": contrasena ,"sexo": sexo1 , "orientacion": os,"tiene_pareja": pareja ,"app_citas" : apps, "dificultad_citas":dificultad, "importancia_hobbies":importancia, "importancia_estudios": imp, "gusto_musical":gusto, "misma_region": region, "gusto_diferente":gustoDif, "Buenos_habitos": Habits, "Metas_similares":goals, "vida_profesional":prof,"gustos_similares": similar}
        generate_user(UserDict)
#Tiene pareja o no tiene contrasena
